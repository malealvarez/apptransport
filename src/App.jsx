...
import {
  ...
  Search, Info, X, Check, Pencil, Trash2, Loader, LogOut
} from "lucide-react";
...
// ─── Componentes UI FUERA de App para evitar pérdida de foco ───────────────

const Badge=({color="#7B8699",bg,children})=>(
...
const Btn=({children,onClick,variant="primary",small,style,disabled,icon,T})=>{
  const v={
    primary:{background:T?.accent||"#8BAAC2",color:"#fff"},
    secondary:{background:T?.surface2||"#F0F1F5",color:T?.sub||"#4C566A",border:`1px solid ${T?.border||"#DDE0E8"}`},
    success:{background:T?.green||"#8ABCAA",color:"#fff"},
    danger:{background:T?.rose||"#C4A0B0",color:"#fff"},
  };
  return <button disabled={disabled} onClick={onClick} style={{borderRadius:10,border:"none",cursor:disabled?"not-allowed":"pointer",fontWeight:600,fontSize:small?BASE*0.78:BASE*0.95,padding:small?"6px 12px":"10px 20px",opacity:disabled?.5:1,display:"inline-flex",alignItems:"center",gap:6,...(v[variant]||v.primary),...style}}>{icon}{children}</button>;
};
...
const Card=({children,style,accent,T})=>(
  <div style={{background:T.surface,borderRadius:16,padding:16,border:`1px solid ${T.border}`,borderLeft:accent?`4px solid ${accent}`:`1px solid ${T.border}`,boxShadow:"0 1px 6px #2E344009",transition:"background .3s",...style}}>{children}</div>
);

const SecTitle=({children,T})=>(
...
const Modal=({title,onClose,children,wide,T,dark})=>(
  <div style={{position:"fixed",inset:0,background:dark?"#00000099":"#2E344088",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 28px",width:`min(100vw,${wide?700:520}px)`,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 -8px 40px #0003",border:`1px solid ${T.border}`}}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const unsubs=[
      ...
      onSnapshot(collection(db,"clients"),snap=>{setClients(snap.docs.map(d=>({id:d.id,...d.data()})));}),
      onSnapshot(collection(db,"obrasSociales"),snap=>{setObrasSociales(snap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false);}),
    ];
  ...
  const handleLogout=async()=>{
    await signOut(auth);
    setDrivers([]);setClients([]);setObrasSociales([]);setLoading(true);
  };
  ...
  const buildGrid=()=>{
    const g={};
    DAYS.forEach(day=>{
      g[day]={};cd.forEach(dr=>{g[day][dr.id]=[];});
      clients.forEach(c=>{c.transfers?.forEach(t=>{if(!t.days?.includes(day))return;if(!g[day][t.driverId])g[day][t.driverId]=[];g[day][t.driverId].push({client:c,transfer:t});});});
  ...
  if(authLoading) return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F2F3F6",gap:16}}>
      ...
      <div style={{color:"#7B8699",fontSize:BASE}}>Iniciando...</div>
    </div>
  );

  if(!user) return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F2F3F6",padding:16}}>
      <style>{spinStyle}</style>
      <div style={{background:"#fff",borderRadius:20,padding:36,width:"min(90vw,380px)",boxShadow:"0 4px 30px #0002",border:"1px solid #DDE0E8"}}>
        ...
        {loginError&&<div style={{background:"#F5EDF1",color:"#C4A0B0",borderRadius:8,padding:"8px 12px",fontSize:13,marginBottom:14,textAlign:"center"}}>{loginError}</div>}
        <button onClick={handleLogin} style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:"#8BAAC2",color:"#fff",fontWeight:700,fontSize:16,cursor:"pointer"}}>Iniciar sesión</button>
      </div>
    </div>
  );
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',sans-serif",background:T.bg,minHeight:"100vh",width:"100%",display:"flex",flexDirection:"column",color:T.text,transition:"background .3s,color .3s",fontSize:BASE}}>
      <style>{`*{box-sizing:border-box}body{margin:0}${spinStyle}@media(max-width:600px){.stats{grid-template-columns:1fr 1fr!important}.btotals{grid-template-columns:1fr!important}.hide-m{display:none!important}}`}</style>

      {/* Header */}
      <div style={{background:T.navBg,padding:"14px 16px 0",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,position:"relative"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>setDark(d=>!d)} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.18)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:20,padding:"6px 12px",cursor:"pointer",color:"#fff",fontSize:BASE*0.72,fontWeight:600}}>
              ...
              <span className="hide-m">{dark?"Claro":"Oscuro"}</span>
            </button>
            <button onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.18)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:20,padding:"6px 12px",cursor:"pointer",color:"#fff",fontSize:BASE*0.72,fontWeight:600}}>
              <LogOut size={14}/>
              <span className="hide-m">Salir</span>
            </button>
          </div>
        </div>
        <div style={{display:"flex",width:"100%",overflowX:"auto",scrollbarWidth:"none"}}>
          {nav.map(n=>(
            <button key={n.key} onClick={()=>setTab(n.key)} style={{flex:"0 0 auto",padding:"9px 10px",border:"none",background:tab===n.key?"rgba(255,255,255,0.2)":"transparent",cursor:"pointer",color:tab===n.key?"#fff":T.navColor,borderBottom:tab===n.key?"3px solid #fff":"3px solid transparent",fontSize:BASE*0.68,fontWeight:tab===n.key?700:500,transition:"all .2s",display:"flex",flexDirection:"column",alignItems:"center",gap:3,whiteSpace:"nowrap"}}>
      ...
      <div style={{flex:1,padding:"16px 24px",width:"100%"}}>
        ...
        {tab==="dashboard"&&(
          <div>
            <div className="stats" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16}}>
              {[
                ...
                <div key={i} style={{background:T.surface,borderRadius:14,padding:"14px 10px",border:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,boxShadow:"0 1px 6px #2E344009"}}>
            ...
            <div style={{background:T.surface,borderRadius:16,border:`1px solid ${T.border}`,overflow:"auto",boxShadow:"0 2px 12px #2E344009"}}>
                                  ...
                                  <div style={{background:dr.color+"28",borderLeft:`3px solid ${dr.color}`,borderRadius:"0 8px 8px 0",padding:"5px 7px"}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                                      ...
                                      <div style={{display:"flex",gap:1}}>
                                        <IconBtn onClick={()=>openEditClient(c)} color={T.accent}><Pencil size={10}/></IconBtn>
                                        <IconBtn onClick={()=>deleteClient(c.id)} color={T.rose}><Trash2 size={10}/></IconBtn>
                                      </div>
                                    </div>
        ...
        {tab==="clients"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              <Btn T={T} icon={<Plus size={15}/>} onClick={openAddClient}>Nuevo</Btn>
            </div>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div style={{fontWeight:700,fontSize:BASE,color:T.text}}>{c.name}</div>
                          <div style={{display:"flex",gap:2}}>
                            <IconBtn onClick={()=>openEditClient(c)} color={T.accent}><Pencil size={14}/></IconBtn>
                            <IconBtn onClick={()=>deleteClient(c.id)} color={T.rose}><Trash2 size={14}/></IconBtn>
                          </div>
                        </div>
                        ...
                        {os&&<div style={{marginTop:5}}><Badge color={T.lavender}>{os.name}</Badge></div>}
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <div style={{background:T.greenBg,borderRadius:10,padding:"8px 12px",textAlign:"center",border:`1px solid ${T.green}33`}}>
                        ...
                        <div style={{background:T.accentBg,borderRadius:10,padding:"8px 12px",textAlign:"center",border:`1px solid ${T.accent}33`}}>
                      ...
                      {c.transfers?.map((t,i)=>{
                        ...
                        return(
                          <div key={i} style={{background:T.surface2,borderRadius:9,padding:"6px 10px",fontSize:BASE*0.84,display:"flex",flexWrap:"wrap",gap:6,alignItems:"center",border:`1px solid ${T.border}`}}>
                            ...
                            {dr&&<Badge color={dr.color}>{dr.name}</Badge>}
        ...
        {tab==="drivers"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              <Btn T={T} icon={<Plus size={15}/>} onClick={openAddDriver}>Nuevo</Btn>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {cd.map(dr=>{
                const total=clients.reduce((s,c)=>s+(c.transfers?.filter(t=>t.driverId===dr.id).length||0),0);
                return(
                  <Card key={dr.id} T={T} accent={dr.color}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div>
                        <div style={{fontWeight:700,fontSize:BASE,color:T.text}}>{dr.name}</div>
                        <div style={{fontSize:BASE*0.84,color:T.muted,marginTop:3,display:"flex",alignItems:"center",gap:4}}><Phone size={12}/>{dr.phone}</div>
                        <div style={{marginTop:7}}><Badge color={dr.color}>{total} traslado{total!==1?"s":""}</Badge></div>
                      </div>
                      <div style={{display:"flex",gap:2}}>
                        <IconBtn onClick={()=>openEditDriver(dr)} color={T.accent}><Pencil size={14}/></IconBtn>
                        <IconBtn onClick={()=>deleteDriver(dr.id)} color={T.rose}><Trash2 size={14}/></IconBtn>
                      </div>
                    </div>
        ...
        {tab==="billing"&&(
          <div>
            ...
            <div className="btotals" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                  ...
                  <Card key={c.id} T={T}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                      <div style={{flex:1}}>
                        ...
                        {os&&<div style={{marginTop:4}}><Badge color={T.lavender}>{os.name}</Badge></div>}
                        ...
                        <div style={{marginTop:8}}><Badge color={T.amber} bg={T.amberBg}>{MONTHS[billingMonth]}</Badge></div>
                      </div>
        ...
        {tab==="os"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              <Btn T={T} icon={<Plus size={15}/>} onClick={openAddOS}>Nueva</Btn>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {obrasSociales.map(os=>{
                const osClients=clients.filter(c=>c.obraSocialId===os.id);
                return(
                  <Card key={os.id} T={T} accent={T.lavender}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div style={{fontWeight:700,fontSize:BASE,color:T.text}}>{os.name}</div>
                          <div style={{display:"flex",gap:2}}>
                    ...
                    {osClients.length>0&&(
                      <div style={{marginTop:10}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{osClients.map(c=><Badge key={c.id} color={T.lavender}>{c.name}</Badge>)}</div>
                      </div>
                    )}
                    {os.files?.length>0&&(
                      <div style={{marginTop:8}}>
      ...
      {/* MODAL CLIENT */}
      {modal==="client"&&(
        <Modal T={T} dark={dark} title={editClientId?"Editar cliente":"Nuevo cliente"} onClose={()=>{setModal(null);setSuggestions(null);setNc(emptyClient);setEditClientId(null);}} wide>
          <SecTitle T={T}>DATOS PERSONALES</SecTitle>
          <Inp T={T} label="Nombre *" value={nc.name} onChange={e=>setNc(c=>({...c,name:e.target.value}))} placeholder="Ana García"/>
          <Inp T={T} label="Teléfono" value={nc.phone} onChange={e=>setNc(c=>({...c,phone:e.target.value}))} placeholder="261-555-0000"/>
          <Inp T={T} label="Domicilio principal *" value={nc.address} onChange={e=>setNc(c=>({...c,address:e.target.value}))} placeholder="Av. San Martín 450, Mendoza"/>
          <Inp T={T} label="Domicilio secundario" value={nc.address2} onChange={e=>setNc(c=>({...c,address2:e.target.value}))} placeholder="Opcional"/>
          <Sel T={T} label="Obra Social" value={nc.obraSocialId} onChange={e=>setNc(c=>({...c,obraSocialId:e.target.value}))}>
            <option value="">Sin obra social</option>
            {obrasSociales.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}
          </Sel>
          <SecTitle T={T}>PRESUPUESTOS</SecTitle>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Inp T={T} label="Real ($)" type="number" value={nc.presupuestoReal} onChange={e=>setNc(c=>({...c,presupuestoReal:e.target.value}))} placeholder="0"/>
            <Inp T={T} label="Facturado ($)" type="number" value={nc.presupuestoFacturado} onChange={e=>setNc(c=>({...c,presupuestoFacturado:e.target.value}))} placeholder="0"/>
          </div>
          <SecTitle T={T}>DESTINOS</SecTitle>
          {nc.destinations.map((d,i)=>(
            <div key={i} style={{background:T.surface2,borderRadius:12,padding:12,marginBottom:10,border:`1px solid ${T.border}`}}>
              <Inp T={T} label={`Destino ${i+1}`} value={d.name} onChange={e=>{const ds=[...nc.destinations];ds[i]={...ds[i],name:e.target.value};setNc(c=>({...c,destinations:ds}));}} placeholder="Colegio San José"/>
              <Inp T={T} label="Dirección" value={d.address} onChange={e=>{const ds=[...nc.destinations];ds[i]={...ds[i],address:e.target.value};setNc(c=>({...c,destinations:ds}));}} placeholder="Belgrano 230" style={{marginBottom:0}}/>
            </div>
          ))}
          <Btn T={T} variant="secondary" small icon={<Plus size={12}/>} onClick={()=>setNc(c=>({...c,destinations:[...c.destinations,{name:"",address:""}]}))} style={{marginBottom:4}}>Agregar destino</Btn>
          {nc.destinations[0]?.address&&(
            <>
              <SecTitle T={T}>AGREGAR TRASLADO</SecTitle>
              <div style={{background:T.surface2,borderRadius:12,padding:12,border:`1px solid ${T.border}`}}>
                <Sel T={T} label="Destino" value={nt.destination} onChange={e=>setNt(t=>({...t,destination:Number(e.target.value)}))}>
                  {nc.destinations.map((d,i)=><option key={i} value={i}>{d.name||`Destino ${i+1}`}</option>)}
                </Sel>
                <Sel T={T} label="Tipo" value={nt.type} onChange={e=>setNt(t=>({...t,type:e.target.value}))}>
                  <option value="ida">↗ Ida</option>
                  <option value="vuelta">↩ Vuelta</option>
                </Sel>
                <Sel T={T} label="Horario" value={nt.time} onChange={e=>setNt(t=>({...t,time:e.target.value}))}>
                ...
                {nt.days.length>0&&<Btn T={T} variant="success" icon={<Search size={14}/>} onClick={checkAvail} style={{width:"100%"}}>Ver choferes disponibles</Btn>}
              </div>
              {suggestions&&(
                        ...
                        :avail.map(dr=>{
                          ...
                          return(
                            <div key={dr.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:T.surface,borderRadius:10,padding:"7px 12px",marginBottom:4,border:`1px solid ${T.border}`}}>
                              ...
                              <Btn T={T} small icon={<Check size={11}/>} onClick={()=>assignDriver(dr.id)}>Asignar</Btn>
                            </div>
                          );
                        })
                      }
          ...
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:16}}>
            <Btn T={T} variant="secondary" onClick={()=>{setModal(null);setSuggestions(null);setNc(emptyClient);setEditClientId(null);}}>Cancelar</Btn>
            <Btn T={T} onClick={saveClient}>{editClientId?"Guardar cambios":"Guardar"}</Btn>
          </div>
        </Modal>
      )}
      ...
      {/* MODAL DRIVER */}
      {modal==="driver"&&(
        <Modal T={T} dark={dark} title={editDriverId?"Editar chofer":"Nuevo chofer"} onClose={()=>{setModal(null);setNd(emptyDriver);setEditDriverId(null);}}>
          <Inp T={T} label="Nombre *" value={nd.name} onChange={e=>setNd(d=>({...d,name:e.target.value}))} placeholder="Juan Pérez"/>
          <Inp T={T} label="Teléfono" value={nd.phone} onChange={e=>setNd(d=>({...d,phone:e.target.value}))} placeholder="261-555-0000"/>
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:14}}>
            <Btn T={T} variant="secondary" onClick={()=>{setModal(null);setNd(emptyDriver);setEditDriverId(null);}}>Cancelar</Btn>
            <Btn T={T} onClick={saveDriver}>{editDriverId?"Guardar cambios":"Guardar"}</Btn>
          </div>
        </Modal>
      )}
      ...
      {/* MODAL OS */}
      {modal==="os"&&(
        <Modal T={T} dark={dark} title={editOSId?"Editar obra social":"Nueva Obra Social"} onClose={()=>{setModal(null);setNos(emptyOS);setEditOSId(null);}} wide>
          <SecTitle T={T}>DATOS</SecTitle>
          <Inp T={T} label="Nombre *" value={nos.name} onChange={e=>setNos(o=>({...o,name:e.target.value}))} placeholder="OSEP"/>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Inp T={T} label="Contacto" value={nos.contact} onChange={e=>setNos(o=>({...o,contact:e.target.value}))} placeholder="Nombre"/>
            <Inp T={T} label="Teléfono" value={nos.phone} onChange={e=>setNos(o=>({...o,phone:e.target.value}))} placeholder="261-000-0000"/>
          </div>
          <Inp T={T} label="Email" type="email" value={nos.email} onChange={e=>setNos(o=>({...o,email:e.target.value}))} placeholder="contacto@ossocial.com"/>
          <Inp T={T} label="Dirección" value={nos.address} onChange={e=>setNos(o=>({...o,address:e.target.value}))} placeholder="Dirección"/>
          <div style={{marginBottom:12}}>
          ...
          <SecTitle T={T}>ARCHIVOS</SecTitle>
          <input ref={fileRef} type="file" style={{display:"none"}} onChange={handleOSFile}/>
          <Btn T={T} variant="secondary" small icon={<Paperclip size={11}/>} onClick={()=>fileRef.current.click()} style={{marginBottom:8}}>Adjuntar</Btn>
          {nos.files?.length>0&&(
          ...
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:12}}>
            <Btn T={T} variant="secondary" onClick={()=>{setModal(null);setNos(emptyOS);setEditOSId(null);}}>Cancelar</Btn>
            <Btn T={T} onClick={saveOS}>{editOSId?"Guardar cambios":"Guardar"}</Btn>
          </div>
        </Modal>
      )}
        ...
        <div style={{position:"fixed",inset:0,background:"#00000077",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:T.surface,borderRadius:16,padding:24,width:"min(90vw,360px)",boxShadow:"0 10px 40px #0004",border:`1px solid ${T.border}`}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <Btn T={T} variant="secondary" onClick={()=>setConfirm(null)}>Cancelar</Btn>
              <Btn T={T} variant="danger" icon={<Trash2 size={14}/>} onClick={confirm.onConfirm}>Eliminar</Btn>
            </div>
          </div>
        </div>
      )}
